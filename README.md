...
CapstoneEvaluation(1).ipynb — complete data loading, cleaning, EDA, feature engineering, model training/tuning, plots, diagnostics, classification, explainability, inventory optimization, sensitivity analysis, and conclusions.
...
Lag, rolling, calendar, price, event, holiday, markdown, and macroeconomic signals improve out-of-time forecasting over seasonal-naive baselines.

A forecast-driven base-stock policy reduces average inventory, working capital, and DIO without materially lowering fill rate or cycle-service level.
...
.
├── CapstoneEvaluation(1).ipynb
├── README.md
├── requirements.txt
└── data/                       # excluded from submission/version control
    ├── m5/
...
Download both Kaggle competitions and unzip them into the paths above. You may need to accept the competition rules and configure the Kaggle API.

Open CapstoneEvaluation(1).ipynb.

Run all cells in order.

For optional computationally intensive models, set RUN_HEAVY_MODELS = True. For a faster first run, leave it False.
...
better M5 holdout WAPE than the 7-day seasonal naive baseline;

ideally MASE below 1 and no major residual bias;

reduced working capital or DIO at an acceptable service level;

qualitatively consistent evidence from the Walmart weekly robustness analysis.
